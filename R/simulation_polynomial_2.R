...
for (i in 1:files)
{
  ...
  if (paste0("polynomial_2_",i,".rds") %in% list.files("Results/") == T)
  {
    res <- readRDS(paste0("Results/polynomial_2_",i,".rds"))
  } else
  {
  ...
  saveRDS(list(results=res$results,time=time),paste0("Results/polynomial_2_",i,".rds"))
  rm(out)
  gc()
}
##
