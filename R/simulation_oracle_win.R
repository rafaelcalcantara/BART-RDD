...
fit <- function(i)
{
  ...
  deg.x <- 5
  deg.w <- 2
  if (lvl==1) w2 <- sin(2*pi*w)
  if (lvl==2) w2 <- sin(3*pi*w)*(w-0.5)/(5+exp(-2*w))
  ...
  model <- lm(y~(exp(x)/(1+exp(x))+w2)*z+poly(x,deg.x,raw=T)*as.factor(as.integer(10*w)%%2==0)+poly(w,deg.w,raw=T),data=reg)
