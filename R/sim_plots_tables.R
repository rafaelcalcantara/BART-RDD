...
matplot(unlist(sapply(1:200, function(i) data$w[test[,i],i])),cbind(unlist(cate),unlist(bart.rdd.cate[[9]])),pch=19)
matplot(unlist(sapply(1:200, function(i) data$w[test[,i],i])),cbind(unlist(cate),unlist(sbart.cate[[9]])),pch=19)
matplot(unlist(sapply(1:200, function(i) data$w[test[,i],i])),cbind(unlist(cate),unlist(tbart.cate[[9]])),pch=19)
matplot(unlist(sapply(1:200, function(i) data$w[test[,i],i])),cbind(unlist(cate),unlist(polynomial.cate[[9]])),pch=19)
###
rmse <- cbind(params,rmse.cate)
for (i in c(3,4,5,7))
{
  plot(rmse$delta.tau,rmse[,i],type="p",col=2,pch=rmse$delta.mu+15,
       ylab=colnames(rmse)[i],xlab=expression(delta[tau]),ylim=c(0,0.8))
  lines(unique(rmse$delta.tau),rmse[,i][rmse$delta.mu==0.5],col=2)
  lines(unique(rmse$delta.tau),rmse[,i][rmse$delta.mu==1],col=2)
  lines(unique(rmse$delta.tau),rmse[,i][rmse$delta.mu==2],col=2)
  grid(nx = NA,
       ny = NULL,
  ...
  lines(unique(rmse$delta.tau),rmse$Oracle[rmse$delta.mu==0.5],col="gray")
  lines(unique(rmse$delta.tau),rmse$Oracle[rmse$delta.mu==1],col="gray")
  lines(unique(rmse$delta.tau),rmse$Oracle[rmse$delta.mu==2],col="gray")
}
