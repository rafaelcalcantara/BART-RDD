...
polynomial.full.cate.rmse <- matrix(0,files,3)
...
for (i in 1:files)
{
  ...
  params[i,] <- c(n,data$rho,data$k1,data$k2)
  ate.cate <- sapply(cate,function(i) sqrt(mean((i-data$tau)^2)))
  ...
  bart.rdd.cate.temp <- sqrt(mapply(function(a,b) mean((a-b)^2),bart.rdd.cate[[i]],cate))/ate.cate
  bart.rdd.cate.rmse[i,] <- c(mean(bart.rdd.cate.temp)-sd(bart.rdd.cate.temp),
                              mean(bart.rdd.cate.temp),
                              mean(bart.rdd.cate.temp)+sd(bart.rdd.cate.temp))
  ...
  sbart.cate.temp <- sqrt(mapply(function(a,b) mean((a-b)^2),sbart.cate[[i]],cate))/ate.cate
  sbart.cate.rmse[i,] <- c(mean(sbart.cate.temp)-sd(sbart.cate.temp),
                           mean(sbart.cate.temp),
                           mean(sbart.cate.temp)+sd(sbart.cate.temp))
  ...
  tbart.cate.temp <- sqrt(mapply(function(a,b) mean((a-b)^2),tbart.cate[[i]],cate))/ate.cate
  tbart.cate.rmse[i,] <- c(mean(tbart.cate.temp)-sd(tbart.cate.temp),
                           mean(tbart.cate.temp),
                           mean(tbart.cate.temp)+sd(tbart.cate.temp))
  ...
  oracle.cate.temp <- sqrt(mapply(function(a,b) mean((a-b)^2),oracle.cate[[i]],cate))/ate.cate
  oracle.cate.rmse[i,] <- c(mean(oracle.cate.temp)-sd(oracle.cate.temp),
                            mean(oracle.cate.temp),
                            mean(oracle.cate.temp)+sd(oracle.cate.temp))
  ...
  polynomial.full.cate.temp <- sqrt(mapply(function(a,b) mean((a-b)^2),polynomial.full.cate[[i]],cate))/ate.cate
  polynomial.full.cate.rmse[i,] <- c(mean(polynomial.full.cate.temp)-sd(polynomial.full.cate.temp),
                                     mean(polynomial.full.cate.temp),
                                     mean(polynomial.full.cate.temp)+sd(polynomial.full.cate.temp))
...
# Aggregating results
## RMSE
rmse.cate <- data.frame(`Oracle`=oracle.cate.rmse,
