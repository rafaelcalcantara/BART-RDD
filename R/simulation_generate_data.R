...
w <- lapply(1:s, function(i) matrix(rnorm(n*p,rep((x[,i]-mean(x[,i])),p)*rho,sqrt(1-rho^2)),n,p))
prog <- sapply(1:s, function(i) mu(x[,i],w[[i]],k1,k3,ate,mu.bar))
cate <- sapply(1:s, function(i) tau(x[,i],w[[i]],ate,k2,tau.bar))
...
cate <- sapply(1:s, function(i) tau(c,w[[i]],ate,k2,tau.bar))
...
if (print.params)
{
  print(paste(c("K1: ","K2: ","K3: ","sigma: ","ATE: ","sd(mu): ","sd(tau): "),
              c(k1,k2,k3,sig_error,mean(cate),
                sd(mu(c,w0,k1,k3,ate,mu.bar)),sd(cate))))
}