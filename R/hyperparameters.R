...
for (j in model)
{
    for (k in xi)
    {
        for (l in nu)
        {
            ...
            {
                index <- index+1
                results.h[[index]] <- fit(11,500,j,k,l,m)
            }
        }
    }
}
...
rmse <- apply(do.call("rbind",lapply(results.h,unlist)),2,function(x) mean(x))
rmse <- data.frame(h=seq(0.05,0.15,0.01),RMSE=rmse)
###
...
## fit <- function(s,n,m,xi,nu,kappa)
## {
##     foreach(i=s,.multicombine=T,.export=c("n","c","Omin","h","Opct","m","n","num_sweeps","burnin","Nmin","p_categorical")) %dopar%
##         {
##             Omin <- i
...
##             {
##                 index <- index+1
##                 results.omin[[index]] <- fit(grid,500,j,k,l,m)
##             }
##         }
##     }
## }
...
## rmse <- apply(do.call("rbind",lapply(results.omin,unlist)),2,function(x) mean(x))
## rmse <- data.frame(N=grid,RMSE=rmse)
## ###
## yticks <- seq(min(rmse$RMSE),max(rmse$RMSE),(-min(rmse$RMSE)+max(rmse$RMSE))/5)
