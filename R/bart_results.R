...
res.mat.1.4[2,] <- res.mat.2.4[2,] <- c("XBART","DGP 1","","","","","")
res.mat.1.4[4,] <- res.mat.2.4[4,] <- c("XBART","DGP 2","","","","","")
res.mat.1.4[6,] <- res.mat.2.4[6,] <- c("XBART","DGP 3","","","","","")
...
res.mat.1.6[2,] <- res.mat.2.6[2,] <- c("XBART","DGP 1","","","","","")
res.mat.1.6[4,] <- res.mat.2.6[4,] <- c("XBART","DGP 2","","","","","")
res.mat.1.6[6,] <- res.mat.2.6[6,] <- c("XBART","DGP 3","","","","","")
...
res.mat.1.10[2,] <- c("XBART","DGP 1","","","","","")
res.mat.2.10[2,] <- c("XBART","DGP 4","","","","","")
res.mat.1.10[4,] <- c("XBART","DGP 2","","","","","")
res.mat.2.10[4,] <- c("XBART","DGP 5","","","","","")
res.mat.1.10[6,] <- c("XBART","DGP 3","","","","","")
res.mat.2.10[6,] <- c("XBART","DGP 6","","","","","")
...
res.mat.het[3,7] <- round(mean(mapply(function(x,y) mean((x[,1]-y$cate)^2),cate.sum.7,data[1:s])),2)
res.mat.het[3,8] <- round(mean(mapply(function(x,y) mean(x[,2]<=y$cate & y$cate<=x[,3]),cate.sum.7,data[1:s])),2)
res.mat.het[3,9] <- round(mean(sapply(cate.sum.7,function(i) mean(i[,3]-i[,2]))),2)
###
rm(data,results,no_cores,readFiles,readFiles2,ate1,ate2,ate3,ate4,ate5,ate6,ate7,cate.sum.7)
save.image("Tables/xbart_results.RData")
####
stopImplicitCluster()
####
