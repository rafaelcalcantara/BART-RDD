predict.bayeslm.fit = function(object, data, burnin = NULL, X = NULL, ...){
  ...
  if(is.null(object$call$formula)){
    ...
    z.names <- grep("z1",colnames(X))
    z.names <- colnames(X)[z.names]
    x.names <- grep("x",z.names)
    z.names <- z.names[-x.names]
    ind <- which(colnames(X) %in% z.names)
    mask <- diag(ifelse(1:ncol(X) %in% ind,1,0))
    pred = X %*% mask %*% post_mean
  }
