...
if (isTRUE(run.barddt))
{
  ...
  bart_model_outputs <- foreach (i = 1:num_chains) %dopar% {
    ...
    barddt.mean.parmlist <- list(num_trees=50, min_samples_leaf=20, alpha=0.95, beta=2,
                                 max_depth=20, sample_sigma2_leaf=FALSE, sigma2_leaf_init = diag(rep(1/50,4)))
  ...
  ## Save the results
  saveRDS(pred,"bart_rdd_posterior_lax.rds")
} else
{
  pred <- readRDS("bart_rdd_posterior_lax.rds")
}
# Figure 7---------------------------------------------------------------------
...
## Plot CART tree
pdf("Figures/cate_gpa_lax_prior.pdf")
par(mfrow=c(1,1))
...
pdf("Figures/contour_lax_prior.pdf",width=9,height=4.5)
